...
#ifdef ZEMAX_INPUT
	memset(zemax_array, 0, sizeof(double)*numphotons*6);
...
{	//read ZEMAX input array for initial photon distribution 
    ...
    if ((fp = fopen(fname, "r"))==NULL) {
		...
		/*
		    if ((fp = fopen("4936_EX_1mmoff_0.dat", "r"))==NULL) {
			printf("cannot open ZEMAX Input File. fatal error. quitting.\n");
			exit(EXIT_FAILURE);
		}
		*/
...
{   //save geometrical information of the photons exiting the surface
	...
	//sprintf(fname, fname_zemaxout);
	//sprintf(fname, "Exited Photons_0_1.dat");
